...
class SensorPublisherOld(
    ...
    suspend fun publish() = withContext(ioDispatcher){
        ...
        try {
            ...
            mqttAsyncClient?.setCallback(this@SensorPublisherOld)
            mqttAsyncClient?.connect(connectionOptions,null,object: MqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken?) {
                    println(asyncActionToken)
                }
    ...
    override fun connectComplete(reconnect: Boolean, serverURI: String?) {
        ...
        sensorEventProvider.provideEventsFor(sensorTypes)
    }
    ...
    fun updateSensorsList(sensorTypes: List<Int>){
        sensorEventProvider.stopProvidingEvents()
        sensorEventProvider.provideEventsFor(sensorTypes)
    }
}
